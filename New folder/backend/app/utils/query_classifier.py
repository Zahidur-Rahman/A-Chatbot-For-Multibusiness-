...
async def is_database_related_query_dynamic(message: str, business_id: str, vector_search_service, conversation_history: List = None) -> bool:
    ...
    """
    try:
        logging.info(f"[DynamicClassifier] INPUTS: message='{message}', business_id='{business_id}', conversation_history={conversation_history}")
        # 1. Get schema context for the query
        schema_results = await vector_search_service.search_schemas(business_id, message, top_k=3)
        logging.info(f"[DynamicClassifier] Schema search results: {schema_results}")
...
CLASSIFICATION:
"""
        logging.info(f"[DynamicClassifier] LLM classification prompt:\n{classification_prompt}")
        ...
        logging.info(f"[DynamicClassifier] LLM raw response: {classification_response}")
        # Parse the response
        classification = classification_response.strip().upper()
        is_database_query = "DATABASE_QUERY" in classification
        
        logging.info(f"[DynamicClassifier] Query: '{message}' | LLM Classification: {classification} | DB Query: {is_database_query}")
        logging.info(f"[DynamicClassifier] Schema matches: {len(schema_results) if schema_results else 0}")
